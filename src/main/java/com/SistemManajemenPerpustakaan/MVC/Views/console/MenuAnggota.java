...
public class MenuAnggota implements PersingkatPanggilData {
    ...
    public static void peminjaman(){
        ...
        loop : while (true){
            ...
            for (int i = 0; i < dataBuku.size(); i++){
                if (dataBuku.get(i).getKetersediaanBuku()){
                    break;
                }
            ...
            subLoop : for (int i = 0; i < dataBuku.size(); i++){
                if (dataBuku.get(i).getKetersediaanBuku()){
                    if (dataBuku.get(i).getJudulBuku().toLowerCase().equals(judulBuku.toLowerCase())){
                        kodeBuku = dataBuku.get(i).getKodeBuku();
                        dataBuku.get(i).setKetersediaanBuku(false);
                        break subLoop;
                    }
                    if (i == dataBuku.size() - 1) {
                        System.out.print("Buku dengan judul tersebut sedang tidak tersedia.\n");
            ...
            subLoop : for (int i = 0; i < dataPengguna.size(); i++){
                if (dataPengguna.get(i).getNamaPengguna().toLowerCase().equals(LoginSystem.getPenggunaSaatIni().getNamaPengguna().toLowerCase())){
                    ...
                    idAnggota = dataPengguna.get(i).getIdPengguna();
                    break subLoop;
                }
            }
        ...
        loop : while (true){
            ...
            subLoop : for (int i = 0; i < dataPeminjaman.size(); i++){
                ...
                for (int j = 0; j < dataPengguna.size(); j++){
                    if (dataPeminjaman.get(i).getIdAnggota().equals(LoginSystem.getPenggunaSaatIni().getIdPengguna())) cocok1 = true;
                }
                for (int j = 0; j < dataBuku.size(); j++){
                    if (dataPeminjaman.get(i).getKodebuku().equals(dataBuku.get(j).getKodeBuku())) {
                        indeksBuku = j;
                        cocok2 = true;
                    }
                }
                if (cocok1 == true && cocok2 == true){
                    System.out.print("\n      "+numIndex+". "+dataBuku.get(indeksBuku).getJudulBuku()+" (Deadline "+DateTimeTools.sisaHariMenujuDeadline(dataPeminjaman.get(i).getDeadlinePeminjaman())+" hari lagi)");
                    numIndex++;
                }
            }
